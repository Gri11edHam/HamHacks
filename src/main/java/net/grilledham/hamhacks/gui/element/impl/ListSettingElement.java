...
import net.grilledham.hamhacks.setting.StringList;
...
public class ListSettingElement extends SettingElement {
	...
	public ListSettingElement(float x, float y, float scale, Field setting, Object obj) {
		super(x, y, MinecraftClient.getInstance().textRenderer.getWidth(SettingHelper.getName(setting).getString()) + 18, scale, setting, obj);
		updateList();
	}
	...
	public void render(MatrixStack stack, int mx, int my, float scrollX, float scrollY, float partialTicks) {
		...
		stack.translate(-dropDownCenterX, -dropDownCenterY, 0);
		mc.textRenderer.drawWithShadow(stack, "<", dropDownX, y + 4, ui.textColor.getRGB());
	...
	public void renderTop(MatrixStack stack, int mx, int my, float scrollX, float scrollY, float partialTicks) {
		...
		stack.push();
		RenderUtil.preRender();
		RenderUtil.pushScissor(x + width - maxWidth, y + height, maxWidth, (height * (stringParts.size() + 1)) * (float)selectionAnimation.get(), scale);
		RenderUtil.applyScissor();
		
		ClickGUI ui = PageManager.getPage(ClickGUI.class);
		RenderUtil.drawRect(stack, x + width - maxWidth, y + height, maxWidth, height * (stringParts.size() + 1), ui.bgColor.getRGB());
