...
public class ClickGUI extends Screen {
	...
	private void drawCategory(MatrixStack matrices, int mouseX, int mouseY, Module.Category category) {
		...
		fillRect(matrices, x, y, x + fullWidth, y + h, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
		fillRect(matrices, x, y - 2, x + fullWidth, y, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().accentColor.getRGB());
		drawStringWithShadow(matrices, textRenderer, category.getText(), x + w - textRenderer.getWidth(category.getText()) - 2, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		...
		matrices.translate(-translateX, -translateY, 0);
		drawStringWithShadow(matrices, textRenderer, "<", (int)(translateX - (textRenderer.getWidth("<") / 2f)), (int)(translateY - (textRenderer.fontHeight) / 2f), net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		matrices.pop();
	...
	private void drawModule(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int fullWidth, int h, Module module) {
		...
		drawStringWithShadow(matrices, textRenderer, module.getName(), x + w - textRenderer.getWidth(module.getName()) - 2, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		...
		matrices.translate(-moduleTranslateX, -moduleTranslateY, 0);
		drawStringWithShadow(matrices, textRenderer, "<", (int)(moduleTranslateX - (textRenderer.getWidth("<") / 2f)), (int)(moduleTranslateY - (textRenderer.fontHeight) / 2f), net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		matrices.pop();
		boolean hovered = mouseX > x && mouseX <= x + w && mouseY > y && mouseY <= y + h;
		boolean dropDownHovered = mouseX > x + w && mouseX <= x + fullWidth && mouseY > y && mouseY <= y + h;
	...
	private void drawIntSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, IntSetting setting, boolean canBeClicked) {
		fillRect(matrices, x, y, x + w, y + h, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
		String text = setting.getValue() + " " + setting.getName();
		drawStringWithShadow(matrices, textRenderer, text, x + w - textRenderer.getWidth(text) - 4, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		boolean hovered = mouseX > x && mouseX <= x + w && mouseY > y && mouseY <= y + h;
		if(hovered) {
	...
	private void drawSelectionSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, SelectionSetting setting, boolean canBeClicked) {
		...
		if(expandedSetting == setting) {
			...
			int optionH = h;
			fillRect(matrices, optionsX, optionsY, optionsX + optionsW, optionsY + optionsH, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
			int i = 0;
			for(Text s : setting.getPossibleValues()) {
				if(!s.equals(setting.getValue())) {
					int optionX = optionsX;
					int optionY = optionsY + optionsH * i;
					drawStringWithShadow(matrices, textRenderer, s.getString(), optionX + optionW - textRenderer.getWidth(s) - 2, optionY + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
	...
	private void drawColorSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, ColorSetting setting, boolean canBeClicked) {
		fillRect(matrices, x, y, x + w, y + h, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
		drawStringWithShadow(matrices, textRenderer, setting.getName(), x + w - textRenderer.getWidth(setting.getName()) - 4 - 18, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
	...
	private void drawStringSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, StringSetting setting, boolean canBeClicked) {
		fillRect(matrices, x, y, x + w, y + h, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
		drawStringWithShadow(matrices, textRenderer, setting.getName(), x + w - textRenderer.getWidth(setting.getName()) - 4, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		boolean hovered = mouseX > x && mouseX <= x + w && mouseY > y && mouseY <= y + h;
		if(hovered) {
			fillRect(matrices, x, y, x + w, y + h, 0x20ffffff);
			if(GLFW.glfwGetMouseButton(MinecraftClient.getInstance().getWindow().getHandle(), 0) == GLFW.GLFW_PRESS && !pressed) {
	...
	private void drawBooleanSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, BoolSetting setting, boolean canBeClicked) {
		fillRect(matrices, x + 2, y, x + w, y + h, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().bgColor.getRGB());
	...
	private void drawKeybindSetting(MatrixStack matrices, int mouseX, int mouseY, int x, int y, int w, int h, KeySetting setting, boolean canBeClicked) {
		...
		drawStringWithShadow(matrices, textRenderer, text, x + w - textRenderer.getWidth(text) - 2, y + 2, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
		boolean hovered = mouseX > x && mouseX <= x + w && mouseY > y && mouseY <= y + h;
		if(hovered) {
			fillRect(matrices, x, y, x + w, y + h, 0x20ffffff);
			if(GLFW.glfwGetMouseButton(MinecraftClient.getInstance().getWindow().getHandle(), 0) == GLFW.GLFW_PRESS && !pressed) {
	...
	private void drawColorPicker(MatrixStack matrices, int mouseX, int mouseY, int x, int y, ColorSetting setting) {
		...
		drawStringWithShadow(matrices, textRenderer, "Chroma", chromaX + textRenderer.fontHeight + 2, chromaY, net.grilledham.hamhacks.modules.render.ClickGUI.getInstance().textColor.getRGB());
